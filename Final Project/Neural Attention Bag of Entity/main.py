...
from data import normalize_text, load_20ng_dataset, load_r8_dataset, load_agnews_dataset
...
@cli.command()
@click.argument('wikipedia2vec_file', type=click.Path(exists=True, dir_okay=False)) #enwiki_20180420_lg1_300d.pkl
@click.argument('entity_linker_file', type=click.Path(exists=True, dir_okay=False)) #enwiki_20180420_entity_linker.pkl
@click.option('--dataset', type=click.Choice(['20ng', 'r8','agnews']), default='20ng') #r8
@click.option('--dataset-path', type=click.Path(exists=True, file_okay=False)) #reuters-21578
@click.option('--dev-size', default=0.05) #What is this parameter?
...
@click.option('--sample_ratio', default=0.2)
@click.option('--dim-size', default=300)

def train_classifier(wikipedia2vec_file, entity_linker_file, dataset, dataset_path, dev_size, sample_ratio, **kwargs):
    print(kwargs)
    if dataset == '20ng':
        data = load_20ng_dataset(dev_size)
    elif dataset == 'agnews':
        data = load_agnews_dataset(dataset_path, dev_ratio=dev_size, sample_ratio=sample_ratio)
    else:
